...
{
    [TestFixture]
    public class RuleOfDeadCellTest
    {
        ...
        public void TestApply()
        {
            ...
            IRule<IGrid<ICell>, ICell> rule = container.Resolve<IRule<IGrid<ICell>, ICell>>("Dead");

            Assert.Throws<ArgumentNullException>(() => rule.Apply(null, cell1));
            Assert.Throws<ArgumentNullException>(() => rule.Apply(grid, null));
            Assert.Throws<InvalidOperationException>(() => rule.Apply(grid, cell1));

            ICell result = rule.Apply(grid, grid[1, 0]);
            Assert.AreEqual(result.IsAlive, true);

            result = rule.Apply(grid, grid[0, 0]);
